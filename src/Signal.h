...
struct Signal
{
	 ...
	 */
	Signal(char t_label, std::vector<std::complex<double>> data) : t_data(std::move(data))
	{
	 ...
	 */
	Signal(char t_label, std::vector<std::complex<double>> data, std::string xtra) : f_data(std::move(data)), xtra(xtra)
	{
		if (std::isalpha(static_cast<unsigned char>(t_label)))
			this->t_label = static_cast<char>(std::tolower(static_cast<unsigned char>(t_label)));
...
private:
	...
	std::vector<std::complex<double>> t_data;		// this signal's data in the time domain
	std::vector<std::complex<double>> f_data;		// this signal's data in the frequency domain
	char t_label = 'x';								// the signal's label in the time domain
	char f_label;									// this signal's label in the frequency domain
};
